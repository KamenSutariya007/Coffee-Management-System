...
const SingleProduct = () => {
  ...
  const wishlistState = useSelector((state) => {
    console.log("Full auth state:", state.auth); // Debug: log the full auth state
    ...
    const fetchData = async () => {
      try {
        await Promise.all([
          ...
          dispatch(getuserProductWishlist()) // Fetch wishlist data
        ]);
      } catch (error) {
              ...
              <div>
                {productState?.images?.[0]?.url ? (
                  ...
                  <img 
                    ...
                    alt={productState?.title || 'Product'}
                    className="img-fluid"
                  />
                )}
              </div>
              ...
              {productState?.images?.map((item, index) => (
                <div key={index}>
                  <img 
                    ...
                    className="img-fluid" 
                    alt={`${productState?.title || 'Product'} - ${index + 1}`} 
                  />
                </div>
              ))}
            </div>
