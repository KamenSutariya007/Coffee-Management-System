  ...
  useEffect(() => {
    ...
    const items = cartState
      ...
      .map(item => ({
        ...
        // Make color optional since we removed it from SingleProduct
        color: item.color?._id || null,
        price: Number(item.price) || 0,
      }));

    console.log('Processed cart items for checkout:', items);
    setCartProductState(items);
  }, [cartState]);

  const checkOutHandler = async () => {
    // Validate cart items before proceeding
    if (!Array.isArray(cartProductState) || cartProductState.length === 0) {
      alert("Your cart is empty or invalid. Please add products before checkout.");
      return;
    }
        ...
        <div className="row">
          <div className="col-7">
            <div className="checkout-left-data">
              <h3 className="website-name">CoffeeShop</h3>
              <nav
              ...
              <p className="user-details total">
                Nirav Nandola (coffeeshop@gmail.in)
              </p>
